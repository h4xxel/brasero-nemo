...
static void nemo_disc_burn_bar_finalize   (GObject *object);

#define NEMO_DISC_BURN_BAR_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), NEMO_TYPE_DISC_BURN_BAR, NemoDiscBurnBarPrivate))

struct NemoDiscBurnBarPrivate
{
        GtkWidget  *button;
        GtkWidget  *title;
        gchar      *icon_path;
...
G_DEFINE_TYPE (NemoDiscBurnBar, nemo_disc_burn_bar, GTK_TYPE_BOX)
...
GtkWidget *
nemo_disc_burn_bar_get_button (NemoDiscBurnBar *bar)
{
        GtkWidget *button;
...
static void
nemo_disc_burn_bar_set_property (GObject            *object,
                                guint               prop_id,
                                const GValue       *value,
                                GParamSpec         *pspec)
{
...
static void
nemo_disc_burn_bar_get_property (GObject    *object,
                                guint       prop_id,
                                GValue     *value,
                                GParamSpec *pspec)
{
...
static void
nemo_disc_burn_bar_class_init (NemoDiscBurnBarClass *klass)
{
        GObjectClass   *object_class = G_OBJECT_CLASS (klass);

        object_class->finalize     = nemo_disc_burn_bar_finalize;
        object_class->get_property = nemo_disc_burn_bar_get_property;
        object_class->set_property = nemo_disc_burn_bar_set_property;

        g_type_class_add_private (klass, sizeof (NemoDiscBurnBarPrivate));

        signals [TITLE_CHANGED] = g_signal_new ("title_changed",
                                                G_TYPE_FROM_CLASS (klass),
                                                G_SIGNAL_RUN_LAST,
                                                G_STRUCT_OFFSET (NemoDiscBurnBarClass, title_changed),
                                                NULL, NULL,
        ...
        signals [ICON_CHANGED] = g_signal_new ("icon_changed",
                                               G_TYPE_FROM_CLASS (klass),
                                               G_SIGNAL_RUN_LAST,
                                               G_STRUCT_OFFSET (NemoDiscBurnBarClass, icon_changed),
                                               NULL, NULL,
                                               g_cclosure_marshal_VOID__VOID,
                                               G_TYPE_NONE, 0);
        signals [ACTIVATE] = g_signal_new ("activate",
                                           G_TYPE_FROM_CLASS (klass),
                                           G_SIGNAL_RUN_LAST,
                                           G_STRUCT_OFFSET (NemoDiscBurnBarClass, activate),
                                           NULL, NULL,
                                           g_cclosure_marshal_VOID__VOID,
                                           G_TYPE_NONE, 0);
}
...
static void
nemo_disc_burn_bar_title_changed (GtkEditable *editable,
                                      NemoDiscBurnBar *bar)
{
	g_signal_emit (bar,
...
static void
nemo_disc_burn_bar_icon_button_clicked (GtkEntry *entry,
                                            ...
                                            NemoDiscBurnBar *bar)
{
	...
	gtk_widget_destroy (chooser);
        ...
        nemo_disc_burn_bar_set_icon (bar, path);
        g_free (path);
...
static void
nemo_disc_burn_bar_title_insert_text (GtkEditable *editable,
                                          const gchar *text,
                                          gint length,
                                          gint *position,
                                          NemoDiscBurnBar *bar)
{
	...
	*current = '\0';
	g_signal_handlers_block_by_func (editable,
					 (gpointer) nemo_disc_burn_bar_title_insert_text,
					 bar);
...
static void
nemo_disc_burn_bar_init (NemoDiscBurnBar *bar)
{
        GtkWidget   *table;
        GtkWidget   *label;
        GtkWidget   *hbox;
        ...
        bar->priv = NEMO_DISC_BURN_BAR_GET_PRIVATE (bar);
        ...
        g_signal_connect (entry,
			  "icon-release",
			  G_CALLBACK (nemo_disc_burn_bar_icon_button_clicked),
			  bar);
	g_signal_connect (entry,
			  "insert_text",
			  G_CALLBACK (nemo_disc_burn_bar_title_insert_text),
			  bar);
	g_signal_connect (entry,
			  "changed",
			  G_CALLBACK (nemo_disc_burn_bar_title_changed),
			  bar);

        /* Translators: be careful, anything longer than the English will likely
         * not fit on small Nemo windows */
...
static void
nemo_disc_burn_bar_finalize (GObject *object)
{
        NemoDiscBurnBar *bar;

        g_return_if_fail (object != NULL);
        g_return_if_fail (NEMO_IS_DISC_BURN_BAR (object));

        bar = NEMO_DISC_BURN_BAR (object);
        ...
        G_OBJECT_CLASS (nemo_disc_burn_bar_parent_class)->finalize (object);
}

GtkWidget *
nemo_disc_burn_bar_new (void)
{
        GObject *result;

        result = g_object_new (NEMO_TYPE_DISC_BURN_BAR,
                               "spacing", 6,
                               NULL);
